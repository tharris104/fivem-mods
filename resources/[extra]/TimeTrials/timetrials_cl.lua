-- Local parameters
local START_PROMPT_DISTANCE = 20.0              -- distance to prompt to start race
...
local RACING_HUD_COLOR = {51, 204, 51, 255}    -- color for racing HUD above map
...
--                    local aheadVehNameText = GetLabelText(GetDisplayNameFromVehicleModel(aheadVehHash))
                    local aheadVehNameText = GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1))))
                    local score = {}
...
--                  -- Print spawncode instead to support all custom vehicles
--                    local mycarname = GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1))))
